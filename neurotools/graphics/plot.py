...
def waterfall_plot(x,y,ax=None,colors=None,dy=0.3,lw=0.5,xlabel='Location',draw_ratelabel=False):
    '''
    Waterfall plot for self-healing codes paper
    https://www.biorxiv.org/content/10.1101/2021.03.08.433413v1
    ...
    Other Parameters
    ----------------
    ax: which axis to use; Default is None, which uses the current axis
    colors: colors of each line in the waterfall plot
    dy: vertical spacing of lines in waterfall plot. default is 0.3
    lw: line width. default is 0.5
    xlabel: x label, default is "Location"
    draw_ratelabel: bool; whether to draw "Mean Rate\n(normalized)" y-label
    '''
    if ax is None: ax=plt.gca()
    N = len(y)
    if colors is None:
    ...
    for i,yi in enumerate(y):
        z = yi+i*dy
        if not all(np.isfinite(z)): continue
        plt.fill_between(x,0,z,facecolor='w',edgecolor='w',lw=lw,zorder=-i,clip_on=False)
        plt.plot(x,z,color=colors[i],zorder=-i,clip_on=False,lw=lw,linestyle='-')
    noxyaxes()
    plt.xticks([0,1])
    plt.yticks([dy,N*dy],['1','%d'%N])
    plt.xlabel(xlabel)
    ...
    if draw_ratelabel:
        ...
        plt.text(1.1+pixels_to_xunits(5),0+dy*0.125,'Mean Rate\n(normalized)',fontsize=7,rotation=90,ha='left',va='bottom')
    plt.xlim(x[0],x[-1])
    plt.ylim(0,dy*(N+3))

def sign_deviation_cmap(vmin,vmax,center,cmap,cmiddle=0.5):
    r = (center - vmin)/(vmax - vmin)
    p = np.interp1d([0,r,1],[0,cmiddle,1],'linear')(np.linspace(0,1,64))
    return matplotlib.colors.LinearSegmentedColormap.from_list('new_map',cmap(p))
